...
class SutdaDeck {
	...
	HashMap<Object, Object> jokbo = new HashMap<Object, Object>(); // 족보를 저장할 HashMap
	...
	int getPoint(Player p) {
		...
		Integer result = 0;
		/*
		 * (2) 아래의 로직에 맞게 코드를 작성하시오. 1. 카드 두 장이 모두 광이면, jokbo에서 키를 "KK"로 해서 점수를
		 * 조회한다. 2. 두 카드의 숫자(num)로 jokbo에서 등급을 조회한다. 3. 해당하는 등급이 없으면, 아래의 공식으로
		 * 점수를 계산한다. (c1.num + c2.num) % 10 + 1000 4. Player의 점수(point)에 계산한 값을
		 * 저장한다.
		 */
		
		// 카드 두장이 모두 광이면 KK로 해서 점수를 조회
